...
#else
word  cdecl crc16upd	(word  FAR *crctab, word  crc, byte c);
#endif
...
/* ------------------------------------------------------------------------- */
void  cdecl crc16rinit	(word  FAR *crctab, word  poly);
word  cdecl crc16rblock (word  FAR *crctab, word  crc, byte FAR *buf, word len);
#ifndef CRC_TINY
#define crc16rupd(crctab,crc,c) \
		  ((crctab)[(((crc) >> 8) ^ (c)) & 0xff] ^ ((crc) << 8))
#else
word  cdecl crc16rupd	(word  FAR *crctab, word  crc, byte c);
#endif

/* ------------------------------------------------------------------------- */
/* CRC-32								     */
/* ------------------------------------------------------------------------- */
void  cdecl crc32init	(dword FAR *crctab, dword poly);
dword cdecl crc32block	(dword FAR *crctab, dword crc, byte FAR *buf, word len);
#ifndef CRC_TINY
#define crc32upd(crctab,crc,c) \
		  ((crctab)[((int) (crc) ^ (c)) & 0xff] ^ ((crc) >> 8))
#else
dword cdecl crc32upd	(dword FAR *crctab, dword crc, byte c);
#endif
